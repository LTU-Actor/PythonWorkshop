...
# Joystick mapping
JOY_BUTTON_A = 0 # Go backward and turn right
...
class GemDrive():
    def __init__(self):
        ...
        self.linear_x_gain = 0.5
        self.steering_gain = 0.15
        self.desired_dist = 6.5
    ...
    def state_machine(self):
        ...
        if( self.vehicle_state == STATES.index('Stop') ):
            ...
            if( self.vehicle_state == STATES.index('Forward') ):
                ...
                if( self.distance - self.start_position < 1.0 ):
            ...
            if( self.vehicle_state == STATES.index('Reverse') ):
                ...
                if( self.distance - self.start_position > -0.75*self.desired_dist ):
                    self.msg_twist_cmd.linear.x = -self.linear_x_gain
                    self.msg_twist_cmd.angular.z = -self.steering_gain
                else:
                    self.initialize_commands()
            ...
            if( self.vehicle_state == STATES.index('Left') or
                ...
                if( self.distance - self.start_position < 0.25 ):
                    self.msg_twist_cmd.linear.x = self.linear_x_gain
                    self.msg_twist_cmd.angular.z = 0.0
